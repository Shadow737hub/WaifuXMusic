...
async def gift(client, message):
    ...
    for _sender_id, _ in pending_gifts.keys():
        if _sender_id == sender_id:
            await message.reply_text(
                "⚠️ You already have a gift processing. Please confirm or cancel it before sending another gift.",
    ...
    if not message.reply_to_message:
        await message.reply_text(
            "ℹ️ You need to reply to a user's message to gift a character!",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    if sender_id == receiver_id:
        await message.reply_text("❌ You can't gift a character to yourself!", parse_mode=ParseMode.MARKDOWN)
        return

    if len(message.command) != 2:
        await message.reply_text("Usage: `/gift <character_id>`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if not sender or 'characters' not in sender:
        await message.reply_text("❌ You don't have any characters in your collection.", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if not character:
        await message.reply_text("❌ You don't have this character in your collection!", parse_mode=ParseMode.MARKDOWN)
        return
...
async def on_callback_query(client, callback_query):
    ...
    if callback_query.data == "confirm_gift":
        ...
        if gift['processed']:
            await callback_query.answer("⚠️ This gift has already been processed.", show_alert=True)
            return
